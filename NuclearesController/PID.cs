...
internal class PID {
    ...
    public double Step(int timeSecondsNow, double target, double actual, double? directDelta = null) {
        ...
        double cP = error * KP * this.inverseEffectFactor;
        double cI = 0;
        double cD = 0;
        double deriv = 0;
        if (directDelta == null) {
        ...
        cD = KD * deriv * this.inverseEffectFactor;
        bool ciLimited = false;
        if (this.lastDatapoint != null) {
            var deltaT = currDatapoint.Item1 - this.lastDatapoint.Value.Item1;
            var unclamped = this.integratorValue + KI * error * deltaT * this.inverseEffectFactor;
        ...
        StateString = $"Curr/Target {actual}/{target} E:{error}";
